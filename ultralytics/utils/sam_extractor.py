...
class OBBox:
    def __init__(self, points: np.ndarray, format, normalized) -> None:
        ...
        assert (
            _points.shape[1] == _dims[format]
        ), f"Invalid bounding box shape: {_points.shape}, must be (N, {_dims[format]})"
        if format == "xyxyxyxy":
...
class DatasetOBBExtractor:
    def __init__(
        self, model, dataset_dir, output_dir, default_class=None, debug=False, yaml_cfg=None, keep_only_classes=False
    ):  # Ship classes for xView
        self.model = model
        ...
        if self.default_class:
            assert (
                self.default_class in self.IDX_NAMES.keys()
            ), f"The default class {self.default_class} is not in the class names"
    ...
    def process(self, idxs=None):
        ...
        for data in tqdm(dataset):
            ...
            if self.keep_only_classes:
                labels_reduced = [
                    label for label in labels if label in self.keep_only_classes
                ]  # Hard coded ship classes
                points.points = [
                    point for point, label in zip(points.points, labels) if label in self.keep_only_classes
                ]
            ...
            # Probe SAM
            res = self.model(
                data["filepath"], bboxes=instances.bboxes, points=points.points, labels=labels, verbose=False
            )
            ...
            labels_ = associate_points_to_boxes(
                points.points, obb_boxes, labels_reduced, self.IDX_NAMES[self.default_class]
            )
            # Write label file
    ...
    def plot(self, res, axes_boxes, obb_boxes, labels, output_label):
        ...
        obb_labels_xywhrclass = np.hstack(
            (obb_boxes_xywhr, np.zeros_like(lb), np.ones_like(lb), lb)
        )  # [xywh, rotation,] track_id, conf, cls
        xyxy_labels = np.hstack(
            (
                axes_boxes,
                np.zeros((axes_boxes.shape[0], 1)),
                np.ones((axes_boxes.shape[0], 1)),
                np.zeros((axes_boxes.shape[0], 1)),
            )
        )
...
if __name__ == "__main__":
    ...
    dat = DatasetOBBExtractor(
        model=model,
        dataset_dir="./datasets/xView-patches",
        output_dir="./datasets/xView-patches/obb-sam",
        default_class="Maritime Vessel",
        debug=debug,
    )
