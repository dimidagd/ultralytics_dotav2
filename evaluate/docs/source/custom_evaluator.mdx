...
Now we can wrap it in a simple wrapper class like in the Scikit-Learn example before. It just has to return a list of dictionaries with the predicted labels. If the polarity is larger than 0 we'll predict positive sentiment and negative otherwise:
...
That class is compatible with the `evaluator` and we can use the same instance from the previous example along with the IMDb test set:
